...
description: "Netlify Deployment Skill workflow skill. Use this skill when the user needs Deploy web projects to Netlify using the Netlify CLI (npx netlify). Use when the user asks to deploy, host, publish, or link a site/repo on Netlify, including preview and production deploys and the operator should rely on the packaged workflow, support pack, troubleshooting notes, and provenance links before merging or handing off."
version: "0.0.1"
category: devops
...
This public intake copy packages `skills/.curated/netlify-deploy` from `https://github.com/openai/skills.git` into the native Omni Skills editorial shape without hiding its origin.
...
- Use when the request clearly matches the imported source intent: Deploy web projects to Netlify using the Netlify CLI (npx netlify). Use when the user asks to deploy, host, publish, or link a site/repo on Netlify, including preview and production deploys.
...
**Symptoms:** The result ignores the upstream workflow in `skills/.curated/netlify-deploy`, fails to mention provenance, or does not use the support pack at all.
...
| `assets` | machine-readable source manifest | `assets/netlify-small.svg` |
