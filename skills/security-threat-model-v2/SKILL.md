...
This public intake copy packages `skills/.curated/security-threat-model` from `https://github.com/openai/skills.git` into the native Omni Skills editorial shape without hiding its origin.
...
- Use when the request clearly matches the imported source intent: Repository-grounded threat modeling that enumerates trust boundaries, assets, attacker capabilities, abuse paths, and mitigations, and writes a concise Markdown threat model. Trigger only when the user explicitly asks....
...
**Symptoms:** The result ignores the upstream workflow in `skills/.curated/security-threat-model`, fails to mention provenance, or does not use the support pack at all.
