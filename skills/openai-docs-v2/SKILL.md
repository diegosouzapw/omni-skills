...
version: "0.0.1"
category: ai-agents
tags: ["openai-docs-v2", "openai-docs", "the", "user", "asks", "how", "build", "openai"]
complexity: advanced
risk: safe
...
This public intake copy packages `skills/.system/openai-docs` from `https://github.com/openai/skills.git` into the native Omni Skills editorial shape without hiding its origin.
...
```text
Use @openai-docs-v2 to handle <task>. Start with the workflow playbook, load only the upstream files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @openai-docs-v2 using the checklist, rubric, playbook, and source manifest, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `skills/.system/openai-docs`, fails to mention provenance, or does not use the support pack at all.
