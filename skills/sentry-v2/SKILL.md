---
name: sentry-v2
description: "Sentry (Read-only Observability) workflow skill. Use this skill when the user needs the user asks to inspect Sentry issues or events, summarize recent production errors, or pull basic Sentry health data via the Sentry API; perform read-only queries with the bundled script and require SENTRYAUTHTOKEN and the operator should rely on the packaged workflow, support pack, troubleshooting notes, and provenance links before merging or handing off."
version: "0.0.1"
category: cli-automation
tags: ["sentry-v2", "sentry", "the", "user", "asks", "inspect", "issues", "events"]
complexity: advanced
risk: safe
...
This public intake copy packages `skills/.curated/sentry` from `https://github.com/openai/skills.git` into the native Omni Skills editorial shape without hiding its origin.
...
- Use when the request clearly matches the imported source intent: the user asks to inspect Sentry issues or events, summarize recent production errors, or pull basic Sentry health data via the Sentry API; perform read-only queries with the bundled script and require SENTRYAUTHTOKEN.
...
**Symptoms:** The result ignores the upstream workflow in `skills/.curated/sentry`, fails to mention provenance, or does not use the support pack at all.
