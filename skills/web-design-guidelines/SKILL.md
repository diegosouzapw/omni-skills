...
This public intake copy packages `skills/web-design-guidelines` from `https://github.com/vercel-labs/agent-skills.git` into the native Omni Skills editorial shape without hiding its origin.
...
- Use when the request clearly matches the imported source intent: Review UI code for Web Interface Guidelines compliance. Use when asked to "review my UI", "check accessibility", "audit design", "review UX", or "check my site against best practices".
...
**Symptoms:** The result ignores the upstream workflow in `skills/web-design-guidelines`, fails to mention provenance, or does not use the support pack at all.
