...
This public intake copy packages `skills/.curated/figma` from `https://github.com/openai/skills.git` into the native Omni Skills editorial shape without hiding its origin.
...
- Use when the request clearly matches the imported source intent: Use the Figma MCP server to fetch design context, screenshots, variables, and assets from Figma, and to translate Figma nodes into production code. Trigger when a task involves Figma URLs, node IDs, design-to-code....
...
**Symptoms:** The result ignores the upstream workflow in `skills/.curated/figma`, fails to mention provenance, or does not use the support pack at all.
